...
func (ct *categoryAPI) UpdateCategory(c *gin.Context) {
	...
	if err := c.BindJSON(&category); err != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse{Error: "Invalid data"})
		return
	}

	if err := ct.categoryService.Update(categoryID, category); err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "category update success"}) // TODO: answer here
}
...
func (ct *categoryAPI) DeleteCategory(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "category delete success"}) // TODO: answer here
}
