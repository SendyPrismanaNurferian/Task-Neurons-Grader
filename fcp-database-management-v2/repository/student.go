...
func (s *studentRepoImpl) FetchWithClass() (*[]model.StudentClass, error) {
	...
	if err := s.db.Table("students").Select("students.*, classes.name AS class_name, classes.professor AS professor, classes.room_number AS room_number").Joins("JOIN classes ON students.class_id = classes.id").
