...
func Login(id string, name string) string {
	...
	for _, Entrystudent := range strings.Split(Students, ", ") {
		Datastudent := strings.Split(Entrystudent, "_")
		if Datastudent[0] == id && Datastudent[1] == name {
			return fmt.Sprintf("Login berhasil: %s (%s)", Datastudent[1], Datastudent[2])
		}
	}
...
func GetStudyProgram(code string) string {
	...
	for _, program := range strings.Split(StudentStudyPrograms, ", ") {
		Datastudent := strings.Split(program, "_")
		if Datastudent[0] == code {
			return Datastudent[1]
		}
	}
