...
type StudentManager interface {
	...
	ChangeStudyProgram(programStudi string) StudentModifier
}

type StudentModifier func(s *Student) error
...
func (sm *InMemoryStudentManager) GetStudents() []Student {
	return sm.students
}
...
func (sm *InMemoryStudentManager) Register(id string, name string, studyProgram string) (string, error) {
	...
	sm.students = append(sm.students, Student{ID: id, Name: name, StudyProgram: studyProgram})
...
func (sm *InMemoryStudentManager) ModifyStudent(name string, fn StudentModifier) (string, error) {
	if name == "" {
...
func (sm *InMemoryStudentManager) ChangeStudyProgram(programStudi string) StudentModifier {
	return func(s *Student) error {
		if _, exists := sm.studentStudyPrograms[programStudi]; !exists {
			return errors.New(fmt.Sprintf("Kode program studi %s tidak ditemukan", programStudi))
		}
